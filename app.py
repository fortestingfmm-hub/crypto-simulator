...
# 隐藏 Streamlit 默认的白边、菜单和页脚，实现真正的全屏沉浸感
...
<head>
    ...
    <title>廿九 - 极速实战终端 V12</title>
    <!-- 【重大升级】使用 V4 最新版本，原生支持 autoSize 解决 Streamlit 容器尺寸异常问题 -->
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, sans-serif; -webkit-tap-highlight-color: transparent; }
        body { background-color: var(--bg-dark); color: var(--text-main); padding-bottom: 100px; overflow-x: hidden; 
        ...
        .container { padding: 16px; max-width: 600px; margin: 0 auto; min-height: 100vh; }
        ...
        /* 强制图表容器拥有高度 */
        #tvchart { width: 100%; height: 350px; position: relative; border-radius: 8px; overflow: hidden; background: #0B0E11; border: 1px solid var(--border); }
        .loading-text { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: var(--primary); font-size: 14px; font-weight: bold; z-index: 10; pointer-events: none;}
        ...
        .chat-input-area { display: flex; gap: 10px; background: var(--bg-card); padding: 12px; border-radius: 16px; border: 1px solid var(--border); margin-top: 10px;}
...
<body>
    ...
    <div class="container">
        ...
        <div id="market" class="tab-content">
            <div class="glass-card" style="padding: 16px 10px;">
                <div class="card-title">🔥 市场全景扫描</div>
        ...
        <div id="portfolio" class="tab-content active">
            ...
            <div class="glass-card" style="padding: 12px;">
                ...
                <!-- K线容器 -->
                <div id="tvchart">
                    <div id="chart-loading" class="loading-text">📡 正在连接行情节点...</div>
                </div>
            </div>
        ...
        <div id="ai" class="tab-content">
            <div class="chat-container">
                <div style="text-align:center; margin-bottom: 15px; padding: 12px; background: rgba(142,68,173,0.1); border: 1px dashed var(--ai-color); border-radius: 12px;">
                    <span style="color:var(--ai-glow); font-weight:900; font-size:14px;">⚡ OpenClaw 智能代理 ⚡</span><br>
                    <span style="color:var(--text-muted); font-size:12px; margin-top:4px; display:block;">支持盘面数据调用与深度推理分析</span>
                </div>
                <div class="chat-box" id="chat-box"></div>
                <div class="chat-input-area">
                    <input type="text" id="chat-input" placeholder="问我：比特币现在的点位能做空吗？" onkeypress="if(event.key==='Enter') sendAI()">
    ...
    <!-- 底部 Tab -->
    <div class="tab-bar">
        <div class="tab-item" onclick="switchTab('market', this)"><span class="tab-icon">📊</span><span>行情全景</span></div>
        <div class="tab-item active" onclick="switchTab('portfolio', this)"><span class="tab-icon">⚔️</span><span>实战终端</span></div>
        <div class="tab-item" onclick="switchTab('ai', this)"><span class="tab-icon" style="color:var(--ai-glow)">🔮</span><span>AI智库</span></div>
    </div>
    ...
    <script>
        ...
        window.onload = () => {
            ...
            if(state.chatHistory.length === 0) {
                appendChat('ai', '你好！我是 **OpenClaw** 智能交易代理。我已获得你的底层环境权限。\n\n问我：“BTC 还能追高吗？” 我会结合行情为你分析！', false);
            }
        };
        ...
        function initChartEngine() {
            ...
            try {
                // 原生 autoSize: true 彻底解决 Streamlit iframe 变化引发的宽高=0崩溃问题
        ...
        async function fetchChartData() {
            ...
            const interval = state.chartTF; // '1m', '15m', '1h', '4h', '1d' 完美对应币安API
            let rawData = null;

            // 轮询节点直到获取成功
            for (let domain of BINANCE_DOMAINS) {
                try {
                    const res = await fetch(`${domain}/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=200`);
                    if (res.ok) { 
                        rawData = await res.json(); 
                        if(rawData && rawData.length > 0) break; 
                    }
                } catch(e) { console.warn("节点请求失败:", domain); }
            }

            if (!rawData) {
                document.getElementById('chart-loading').innerText = '⚠️ 无法获取 K线数据，请检查网络或代理';
                return;
            }
            ...
            try {
                ...
                rawData.forEach(d => {
                    const t = Math.floor(d[0] / 1000); // 严格转为秒级整数
                    if (t > lastTimestamp) {
                        cleanData.push({
                            time: t,
                            open: parseFloat(d[1]),
                ...
                state.chartInst.timeScale().fitContent(); // 自动居中自适应
                
                document.getElementById('chart-loading').style.display = 'none';
            } catch (error) {
                console.error("K线数据渲染报错:", error);
                document.getElementById('chart-loading').innerText = '⚠️ 图表渲染出错';
            }
        }
        ...
        async function sendAI() {
            ...
            try {
                let messages =[{ role: 'system', content: `你是 OpenClaw 智能交易助理。回答要专业自信，不要拒绝预测，如果有交易建议请给出确切入场、止盈止损点。` }];
                ...
                document.getElementById(typingId).remove(); appendChat('ai', "⚠️ 连接超时，请重试。"); 
            }
        }
    </script>
</body>
...
# 将 HTML 容器加载到 Streamlit，设为比较宽裕的 height 以容纳所有元素
