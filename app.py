...
<style>
    ...
    /* 苹果风格卡片 */
    .apple-card { background: #FFFFFF; border-radius: 24px; padding: 20px; margin-bottom: 15px; box-shadow: 0 4px 25px rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.01); }
    ...
    .stButton>button { border-radius: 12px; background-color: #007AFF; color: white; font-weight: 600; border: none; height: 42px; }
...
if not st.session_state.logged_in:
    ...
    with t1:
        with st.form("l_f"):
            u = st.text_input("用户名").strip(); p = st.text_input("密码", type="password").strip()
            if st.form_submit_button("登录", use_container_width=True):
                hp = hashlib.sha256(p.encode()).hexdigest(); db = load_db()
    ...
    with t2:
        with st.form("r_f"):
            nu = st.text_input("设置用户名").strip(); np = st.text_input("设置密码", type="password").strip()
...
with c_u: st.markdown(f"**{st.session_state.username}** · 极速仿真终端")
with c_s:
    with st.popover("⚙️"):
        if st.button("🔄 重置资金"): st.session_state.update({"balance":5000000.0,"positions":[],"history":[]}); sync_data(); st.rerun()
        if st.button("🚪 安全退出"): st.query_params.clear(); st.session_state.clear(); st.rerun()
...
with st.container():
    ...
    u_q = a1.text_input("咨询建议...", key="ds_input", label_visibility="collapsed")
    if a2.button("发送", use_container_width=True) and u_q:
        try:
            sys_msg = "你是中文加密专家。简短专业建议。"
...
with tabs[0]:
    ...
    with m_tabs[0]: render_market_list(m_df[m_df['symbol'].isin(st.session_state.favorites)], "fav")
    with m_tabs[1]: render_market_list(m_df.sort_values('priceChangePercent', ascending=False).head(20), "gain")
    with m_tabs[2]: render_market_list(m_df.sort_values('quoteVolume', ascending=False).head(15), "main")
    with m_tabs[3]: render_market_list(m_df.tail(15), "new")
    with m_tabs[4]: render_market_list(m_df[m_df['symbol'].isin(["UNIUSDT","CAKEUSDT","AAVEUSDT","CRVUSDT"])], "dex")
...
def fragment_positions():
    ...
    m_snap = get_market_snapshot()
    for i, pos in enumerate(st.session_state.positions):
        now_p = m_snap[m_snap['symbol'] == pos['交易对']]['lastPrice'].values[0] if not m_snap.empty and pos['交易对'] in m_snap['symbol'].values else pos['开仓价']
        ...
        with st.container():
            ...
            # 🌟 这里的核心：为每个持仓增加迷你 K 线图
            st.components.v1.html(f"""
                <div id="chart_{pos['交易对']}_{i}" style="height:120px; border-radius:12px; overflow:hidden;"></div>
                <script src="https://s3.tradingview.com/tv.js"></script>
                <script>
                new TradingView.widget({{
                  "autosize": true, "symbol": "BINANCE:{pos['交易对']}", "interval": "15", "timezone": "Etc/UTC",
                  "theme": "light", "style": "2", "locale": "zh_CN", "hide_top_toolbar": true, "hide_legend": true,
                  "save_image": false, "container_id": "chart_{pos['交易对']}_{i}"
                }});
                </script>
            """, height=120)
