...
# 5. 顶置 UI 与 心跳注入
# ==========================================
# 强制页面每 5 秒静默刷新一次 Python 状态 (总权益)
st.components.v1.html("""
    <script>
        window.parent.document.addEventListener('keydown', function(e) { if(e.key==='Enter') return; }); // 防止回车干扰
        setTimeout(function(){ window.parent.location.reload(); }, 5000); 
    </script>
""", height=0)

c_u, c_s = st.columns([7, 2])
with c_u: st.markdown(f"**{st.session_state.username}** · 极速模式")
with c_s:
    with st.popover("⚙️"):
...
with st.container():
    ...
    u_q = a1.text_input("咨询 AI...", key="ds_q", label_visibility="collapsed")
    if a2.button("发送", use_container_width=True) and u_q:
        try:
            sys_msg = f"你是中文专家。总权益:{net_equity:.0f}U。行情:{market_df.head(2).to_string()}。简短专业。"
...
# 6. 功能标签页
# ==========================================
tabs = st.tabs(["📊 毫秒行情", "📈 极速交易", "💼 持仓管理", "📜 历史记录"])
...
with tabs[0]:
    ...
    with m_sub[4]: render_list(market_df[market_df['symbol'].isin(["UNIUSDT","CAKEUSDT","AAVEUSDT","CRVUSDT"])], "dex")
    ...
    if not st.session_state.positions: st.info("暂无持仓")
        ...
        for i, pos in enumerate(st.session_state.positions):
            ...
            with st.container():
                ...
                with m2:
                    with st.popover("调仓"):
                        adj = st.slider("金额", 0.0, float(max(st.session_state.balance, pos['占用保证金'])), 500.0, key=f"as_{i}")
