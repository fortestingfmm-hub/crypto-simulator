...
<head>
    ...
    <title>交易币模拟</title>
    <!-- 使用 Lightweight Charts V4 最新版，完美支持 autoSize 防止容器崩溃 -->
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .loading-overlay { position: absolute; inset: 0; background: rgba(11,14,17,0.8); display: flex; justify-content: center; align-items: center; z-index: 10; font-weight: bold; color: var(--primary);}
        ...
        /* 底部导航 */
        .tab-bar { position: fixed; bottom: 0; left: 0; right: 0; height: 80px; background: rgba(11, 14, 17, 0.95); backdrop-filter: blur(15px); display: flex; justify-content: space-around; align-items: center; border-top: 1px solid rgba(255,255,255,0.05); z-index: 100;}
...
<body>
    ...
    <div class="container">
        ...
        <div id="portfolio" class="tab-content active">
            ...
            <div class="glass-card" style="padding: 12px;">
                ...
                <div id="tvchart">
                    <div id="chart-loading" class="loading-overlay">📡 正在连接行情节点...</div>
                </div>
            </div>

            <div class="glass-card">
                <div class="card-title">⚔️ 实战量化 (强平机制启动)</div>
    ...
    <div class="tab-bar">
        ...
        <div class="tab-item active" onclick="switchTab('portfolio', this)"><span class="tab-icon">⚔️</span><span>实战模拟</span></div>
    ...
    <script>
        ...
        // 智能节点：首选免墙公网 vision 节点，杜绝 403 报错
        ...
        window.onload = () => {
            ...
            if(state.chatHistory.length === 0) {
                appendChat('ai', '你好！我是 **OpenClaw** 智能交易代理。欢迎来到**交易币模拟**终端，我已获得你的底层环境权限。\n\n你可以像朋友一样跟我闲聊，探讨宏观经济；或者直接问我：“比特币现在还能追高吗？”\n我都会为你提供最有价值的解答！', false);
            }
        };
        ...
        async function fetchChartHistory() {
            ...
            for (let domain of BINANCE_DOMAINS) {
                try {
                    const res = await fetch(`${domain}/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=250`);
                    if (res.ok) { 
            ...
            if (!rawData) {
                loadingEl.innerHTML = '<span style="color:var(--down-red)">⚠️ 行情数据获取失败，请重试</span>';
                return;
            }
            ...
            try {
                ...
                // 【重要修复】直接 setData 覆盖旧数据，绝不调用会导致崩溃的 removeSeries
                ...
                console.error("渲染 K 线出错:", e); 
                loadingEl.innerHTML = '<span style="color:var(--down-red)">⚠️ 图表数据处理异常</span>';
            }
        }
        ...
        function executeTrade() {
            ...
            showToast(`✅ 成功建仓：${coin} ${side==='long'?'多单':'空单'} ${lev}X`, 'success');
        ...
        function saveTPSL() {
            ...
            const p = state.positions.find(x => x.id === id); if(!p) return closeTPSLModal();

            p.tp = parseFloat(document.getElementById('modal-tp').value) || null;
            p.sl = parseFloat(document.getElementById('modal-sl').value) || null;

            localStorage.setItem('mock_pos', JSON.stringify(state.positions));
            closeTPSLModal(); renderPositions(); showToast(`✅ 止盈止损修改成功！`, 'success');
        }
        ...
        function triggerClose(index, reason, exitPrice = null) {
            ...
            showToast(`🔔 ${p.coin} 触及${reason}平仓。盈亏: ${pnl>=0?'+':''}${pnl.toFixed(2)} U`, pnl>=0?'success':'error');
        ...
        function triggerLiquidation(index) {
            ...
            if (!p.sl) { pnl = -p.margin; state.balance = 0; reasonStr = '💀 爆仓归零'; showToast(`💀 爆仓警告！余额已被清零！`, 'error'); } 
            else { finalPrice = p.sl; pnl = p.side === 'long' ? (p.sl - p.entry)*p.amount : (p.entry - p.sl)*p.amount; state.balance += pnl; reasonStr = '⚠️ 强平截断'; showToast(`⚠️ 触发底线！已强行截断亏损！`, 'error'); }
        ...
        function manualClose(id) { const index = state.positions.findIndex(p => p.id === id); if(index > -1) triggerClose(index, '手动市价平仓'); }
        ...
        function renderPositions() {
            ...
            if(state.positions.length === 0) { container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:30px;font-weight:bold;">您当前没有任何持仓</div>'; return; }
                ...
                <div class="pos-card">
                    ...
                    <div class="pos-grid">
                        ...
                        <div class="grid-item" style="cursor:pointer; background:rgba(14,203,129,0.05); padding:6px; border-radius:6px;" onclick="openTPSLModal('${p.id}')">
                            <span style="color:var(--text-muted); font-size:11px;">止盈 (TP) ✏️</span><span class="grid-val up">${p.tp ? '$'+p.tp : '点击设置'}</span>
                        </div>
                        <div class="grid-item" style="cursor:pointer; background:rgba(246,70,93,0.05); padding:6px; border-radius:6px;" onclick="openTPSLModal('${p.id}')">
                            <span style="color:var(--text-muted); font-size:11px;">止损 (SL) ✏️</span><span class="grid-val down">${p.sl ? '$'+p.sl : '点击设置'}</span>
                        </div>
                    </div>
                    <div class="pnl-row">
        ...
        function renderHistory() {
            ...
            if(state.history.length === 0) { container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:30px;font-weight:bold;">暂无交割单记录</div>'; return; }

            let html = state.history.map(h => {
                const isProfit = h.realizedPnl >= 0; const color = isProfit ? 'var(--up-green)' : 'var(--down-red)'; const sign = isProfit ? '+' : '';
                return `
                <div class="pos-card" style="opacity: 0.9;">
                    ...
                    <div class="pos-grid" style="margin-bottom: 8px;">
                        ...
                        <div class="grid-item"><span>平仓时间</span><span class="grid-val" style="font-weight:normal;">${formatTime(h.closeTime)}</span></div>
                    </div>
        ...
        function clearHistory() { if(confirm('确定要清空这台设备上的所有历史记录吗？')) { state.history =[]; localStorage.setItem('mock_history', '[]'); renderHistory(); } }
        ...
        async function sendAI() {
            ...
            box.innerHTML += `<div class="msg-row ai" id="${typingId}"><div class="msg-bubble" style="color:var(--ai-glow); font-family:monospace;">思考分析中<span class="pulse" style="display:inline-block; margin-left:8px;"></span></div></div>`;
            box.scrollTop = box.scrollHeight;

            try {
                let messages =[{ role: 'system', content: `你是基于 OpenClaw 的智能交易代理。支持自然对话，提供极高的同理心和准确盘面点位分析。` }];
                ...
                let envData = coinRef ? `当前 ${coinRef} 实时价: $${state.prices[coinRef] || '未知'}` : `BTC 实时价: $${state.prices['BTC'] || '未知'}`;
                
                messages.pop(); 
                messages.push({ role: 'user', content: `[注入数据: ${envData}]\n\n用户: ${text}` });

                const res = await fetch('https://text.pollinations.ai/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: messages, model: "openai" }) });

                document.getElementById(typingId).remove();
                if(!res.ok) throw new Error('API Reject');
                appendChat('ai', await res.text());
            } catch(e) { document.getElementById(typingId).remove(); appendChat('ai', "⚠️ 底层连接超时，请重试。"); }
        }
...
# ==========================================
# 3. 将 HTML 嵌入到 Streamlit 中渲染
# ==========================================
# 设置合理的高度，足以装下市场概括列表、实战K线图、持仓单
